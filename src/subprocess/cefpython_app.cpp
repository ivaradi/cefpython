                                    ...
                                    CefRefPtr<CefV8Context> context)
{
    ...
    */
    ...
    if (jsBindings.get())
    {
        ...
        {
            if (jsBindings->HasKey("bindToFrames") && jsBindings->GetType("bindToFrames") == VTYPE_BOOL && jsBindings->GetBool("bindToFrames"))
            {
                DoJavascriptBindingsForFrame(browser, frame, context);
            }
        }
                                            ...
                                            CefRefPtr<CefProcessMessage> message)
{
    ...
    {
        if (args->GetSize() == 1 && args->GetType(0) == VTYPE_DICTIONARY && args->GetDictionary(0)->IsValid())
        {
                                        ...
                                        const CefString &functionName)
{
    ...
    if (std::string::npos != dotPosition)
    {
        ...
        CefString methodName(strFunctionName.substr(dotPosition + 1,
                                                    std::string::npos));
        if (!(jsBindings->HasKey("objects") && jsBindings->GetType("objects") == VTYPE_DICTIONARY))
        {
    ...
    else
    {
        ...
        if (!(jsBindings->HasKey("functions") && jsBindings->GetType("functions") == VTYPE_DICTIONARY))
        {
            LOG(ERROR) << "[Renderer process] BindedFunctionExists():"
                                                ...
                                                CefRefPtr<CefV8Context> context)
{
    ...
    LOG(INFO) << "[Renderer process] DoJavascriptBindingsForFrame():"
                 " bindings are set";
    if (!(jsBindings->HasKey("functions") && jsBindings->GetType("functions") == VTYPE_DICTIONARY && jsBindings->HasKey("properties") && jsBindings->GetType("properties") == VTYPE_DICTIONARY && jsBindings->HasKey("objects") && jsBindings->GetType("objects") == VTYPE_DICTIONARY && jsBindings->HasKey("bindToFrames") && jsBindings->GetType("bindToFrames") == VTYPE_BOOL))
    {
    ...
    if (!(functions->IsValid() && properties->IsValid() && objects->IsValid()))
    {
        LOG(ERROR) << "[Renderer process] DoJavascriptBindingsForFrame():"
                      " invalid data [2]";
        if (didEnterContext)
         ...
         it != objectsVector.end(); ++it)
    {
             ...
             it != methodsVector.end(); ++it)
        {
            ...
            std::string fullMethodName = objectName.ToString().append(".").append(methodName.ToString());
